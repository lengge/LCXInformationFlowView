//
//  LCXInformationFlowSectionHeader.m
...
@interface LCXInformationFlowSectionHeader ()
@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, strong) NSMutableArray<LCXInformationFlowItemView *> *itemViews;
@end

@implementation LCXInformationFlowSectionHeader
...
- (void)prepareForReuse {
    ...
    for (LCXInformationFlowItemView *itemView in self.itemViews) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(header:removeItemView:)]) {
            [self.delegate header:self removeItemView:itemView];
        }
...
- (void)reloadDataWithContentOffset:(CGPoint)contentOffset {
    ...
    CGFloat x = 0;
    CGFloat h = [self.delegate heightOfHeader:self];
    NSInteger columns = [self.dataSource headerNumberOfColumns:self];
    for (NSInteger column = 0; column < columns; column++) {
        LCXInformationFlowItemView *itemView = [self.dataSource header:self itemViewForColumn:column];
        CGFloat width = [self.delegate header:self widthForColumn:column];
    ...
    [self.associatedScrollView setContentSize:CGSizeMake(x, h)];
}
